...
function pairUnits(cards: Card[], levelRank: Rank, trumpSuit: Suit): number {
  const counts = new Map<string, number>();
  for (const card of cards) {
    const pk = pairKey(card, levelRank, trumpSuit);
    counts.set(pk, (counts.get(pk) ?? 0) + 1);
...
export function validateFollowPlay(
  ...
  if (leadPat.kind === 'THROW') {
    // THROW follow: still strictly follow suitGroup/count first.
    ...
    if (groupCards.length >= leadPat.size) {
      ...
      if (leadPairUnits > 0) {
        const availablePairUnits = pairUnits(groupCards, levelRank, trumpSuit);
        const requiredPairUnits = Math.min(leadPairUnits, availablePairUnits);
        const playedPairUnits = pairUnits(playedGroup, levelRank, trumpSuit);
  ...
  if (pairUnits(groupCards, levelRank, trumpSuit) === 0) {
    return { ok: true };
  }
